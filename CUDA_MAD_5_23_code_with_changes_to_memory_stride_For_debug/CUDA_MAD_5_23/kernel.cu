...
void linearize_and_cast_from_Mat_to_float(const cv::Mat& mat_in, float* h_float)
{
	for (int row = 0; row < 512; row++)
	{
		for (int col = 0; col < 512; col++)
		{
			...
			if (row == 0){
				std::cout <<h_float[row * 512 + col]<<"\n";
			}
		}
	}
}
//=============================================================================
__global__ void A1_buildGabor(float* logGabor, const int orientIdx, const int scaleIdx)
{
...
//=============================================================================
__global__ void A3_pointWise_complex_matrix_mult_kernel_2d(cufftComplex* img_spectrum, float* real_filter, cufftComplex* out)
{
	// Grab indices
...
//=============================================================================
__global__ void D3_pointWise_complex_matrix_mult_kernel_2d(cufftComplex* img_spectrum, float* real_filter, cufftComplex* out)
{
	// Grab indices
	int index_x = threadIdx.x + blockIdx.x * blockDim.x;
	int index_y = threadIdx.y + blockIdx.y * blockDim.y;
...
//=============================================================================
__global__ void A4_magnitude_kernel(cufftComplex* d_inverse_complex, float* d_inverse_mag)
{
	// Grab indices
	int index_x = threadIdx.x + blockIdx.x * blockDim.x;
	int index_y = threadIdx.y + blockIdx.y * blockDim.y;
...
//=============================================================================
__global__ void D0_part1_yPlane_CSF_kernel(float* yPlane)
{
...
//=============================================================================
__global__ void D0_part2_xPlane_CSF_kernel(float* xPlane)
{
	float temp = (2 * 32 / 512.0f);
	float tempvar = -(256.0f) - 1 + 0.5f;
	tempvar = tempvar * temp;
...
//=============================================================================
__global__ void D1_map_to_luminance_domain_kernel1(float* float_img_in, float* L_hat)
{
	// Grab indices
	int index_x = threadIdx.x + blockIdx.x * blockDim.x;
	int index_y = threadIdx.y + blockIdx.y * blockDim.y;
...
//=============================================================================
__global__ void D0_part3_build_CSF_kernel(float* csf, const float* yPlane, const float* xPlane)
{
...
//=============================================================================
__global__ void D0_part_4_fftShift_kernel(float* img)
{
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i < 256)
	{
	...
	float* dst_outStd, float* dst_outSkw, float* dst_outKrt, float scale, float* eta)
{
	...
	float delta_stat1 = abs(ref_outStd[threadId] - dst_outStd[threadId]);
	float delta_stat2 = abs(ref_outSkw[threadId] - dst_outSkw[threadId]);
	float delta_stat3 = abs(ref_outKrt[threadId] - dst_outKrt[threadId]);
	eta[grid_index] += scale*(delta_stat1 + 2 * delta_stat2 + delta_stat3);
}
//=============================================================================
//This is the project for optimizing memory access stride.
...
__global__ void A5_fast_lo_stats_kernel(float* xVal, float* outStd, float* outSkw, float* outKrt)
{
	...
	*/
	...
	xVal_smem[threadIdx.y][threadIdx.x] = xVal[global_idy* N + global_idx];
	xVal_smem[threadIdx.y + WIN_SIZE][threadIdx.x] = xVal[global_idy* N + global_idx + WIN_SIZE];
	xVal_smem[threadIdx.y][threadIdx.x + WIN_SIZE] = xVal[(global_idy + WIN_SIZE)* N + global_idx];
	xVal_smem[threadIdx.y + WIN_SIZE][threadIdx.x + WIN_SIZE] = xVal[(global_idy + WIN_SIZE)* N + global_idx + WIN_SIZE];
...
//=================================================================
__global__ void D6_and_D7_zeta_map_kernel(float* outMean, float* outStd, float* outStdMod, float* zeta)
{
	// Grab indices
	int index_x = threadIdx.x + blockIdx.x * blockDim.x;
	int index_y = threadIdx.y + blockIdx.y * blockDim.y;
...
//=============================================================================
__global__ void D8_LMSE_map_kernel(float* reflut, float* D)
{
	// This is kernel #D14
...
//=============================================================================
__global__ void D5_part1_fast_hi_stats_kernel1(float* absRefs, float* absDsts, float* outStd, float* outStdMod, float* outMean, float* ref_img, float* dst_img, float* TMP)
{
...
//=============================================================================
__global__ void D5_part2_fast_hi_stats_kernel2(float* absRefs, float* absDsts, float* outStd, float* outStdMod, float* outMean, float* ref_img, float* dst_img, float* TMP)
{
	//Declarations
	float mean, mean2, stdev;
...
//=============================================================================
__global__ void D9_product_array_kernel(float* out, float* in1, float* in2)
{
...
void write_to_file_DEBUG(float* w, const int SIZE)
{
	...
	for (int i = 0; i < 1; i++)  // Itterate over rows
	{
...
void kernel_wrapper(const cv::Mat& mat_ref, const cv::Mat& mat_dst)
{
	...
	gpuErrchk(cudaMemcpy(d_img_ref_float, h_img_ref_float, REAL_SIZE, cudaMemcpyHostToDevice)) // HOST -> DEVICE
		...
		D1_map_to_luminance_domain_kernel1 << < gridSize, blockSize, 0, stream[1] >> >(d_img_ref_float, d_L_hat_ref);
	cudaCheckError()
		D1_map_to_luminance_domain_kernel1 << < gridSize, blockSize, 0, stream[1] >> >(d_img_dst_float, d_L_hat_dst);
	cudaCheckError()

		float h_L_hat_ref[IMG_SIZE];
	cudaMemcpy(h_L_hat_ref, d_img_ref_float, REAL_SIZE, cudaMemcpyDeviceToHost);
	write_to_file_DEBUG(h_L_hat_ref, N);
	//cudaMemcpy(h_L_hat_dst, d_L_hat_dst, REAL_SIZE, cudaMemcpyDeviceToHost);
	//write_to_file_DEBUG(h_L_hat_dst, N);
	...
	cudaCheckError()
		...
		// Build CSF on Device
		D0_part1_yPlane_CSF_kernel << < 1, 1, 0, stream[1] >> >(d_yPlane);
	cudaCheckError()
		D0_part2_xPlane_CSF_kernel << < 1, 1, 0, stream[1] >> >(d_xPlane);
	cudaCheckError()
		D0_part3_build_CSF_kernel << < gridSize, blockSize, 0, stream[1] >> >(d_CSF, d_yPlane, d_xPlane);
	cudaCheckError()
		D0_part_4_fftShift_kernel << < fftShift_grid_size, fftShift_block_size, 0, stream[1] >> >(d_CSF);
	cudaCheckError()
	...
	cudaCheckError()
		// Filter images
		D3_pointWise_complex_matrix_mult_kernel_2d << < gridSize, blockSize, 0, stream[1] >> >(d_L_hat_ref_complex, d_CSF, d_L_hat_ref_complex);
	cudaCheckError()
		D3_pointWise_complex_matrix_mult_kernel_2d << < gridSize, blockSize, 0, stream[1] >> >(d_L_hat_dst_complex, d_CSF, d_L_hat_dst_complex);
	cudaCheckError()
		// LMSE - CAN MOVE ALMOST ANYWHERE
		D8_LMSE_map_kernel << <gridSize, blockSize, 0, stream[1] >> >(d_reflut, d_lmse);
	cudaCheckError()
#ifdef DEBUG_MODE
	...
	cudaCheckError()
		...
		D5_part1_fast_hi_stats_kernel1 << <loStats_Grid_size, loStats_Block_size, 0, stream[1] >> >(d_I_prime_err, d_I_prime_org, d_outStd, d_outStdMod, d_outMean, d_img_ref_float, d_img_dst_float, d_TEMP);
	cudaCheckError()
		D5_part2_fast_hi_stats_kernel2 << <loStats_Grid_size, loStats_Block_size, 0, stream[1] >> >(d_I_prime_err, d_I_prime_org, d_outStd, d_outStdMod, d_outMean, d_img_ref_float, d_img_dst_float, d_TEMP);
	cudaCheckError()
		D6_and_D7_zeta_map_kernel << <gridSize, blockSize, 0, stream[1] >> >(d_outMean, d_outStd, d_outStdMod, d_zeta);
	cudaCheckError()
		// Product inside summation in MAD eq. 7
		D9_product_array_kernel << <gridSize, blockSize, 0, stream[1] >> >(d_product, d_zeta, d_lmse);
#ifdef DEBUG_MODE
	...
	for (int o = 0; o < 4; o++)
	{
		for (int s = 0; s < 5; s++)
		{
			A1_buildGabor << < gridSize, blockSize, 0, stream[1] >> >(d_logGabor, o, s);
#ifdef DEBUG_MODE
			float gabor[IMG_SIZE];
			cudaMemcpy(gabor, d_logGabor, REAL_SIZE, cudaMemcpyDeviceToHost);
#endif
			cudaCheckError()
				A2_fftShift_kernel << < fftShift_grid_size, fftShift_block_size, 0, stream[1] >> >(d_logGabor);
#ifdef DEBUG_MODE
			cudaMemcpy(gabor, d_logGabor, REAL_SIZE, cudaMemcpyDeviceToHost);
#endif
			cudaCheckError()

				A3_pointWise_complex_matrix_mult_kernel_2d << < gridSize, blockSize, 0, stream[1] >> >(d_ref_cufft, d_logGabor, d_ref_c);
#ifdef DEBUG_MODE
			cufftComplex h_ref_c1[COMPLEX_SIZE];
			cudaMemcpy(h_ref_c1, d_ref_c, COMPLEX_SIZE, cudaMemcpyDeviceToHost);
#endif
			cudaCheckError()
				A3_pointWise_complex_matrix_mult_kernel_2d << < gridSize, blockSize, 0, stream[1] >> >(d_dst_cufft, d_logGabor, d_dst_c);
#ifdef DEBUG_MODE
			...
			cudaCheckError()
				...
				A4_magnitude_kernel << < gridSize, blockSize, 0, stream[1] >> >(d_ref_c, d_ref_c_mag);
//#ifdef DEBUG_MODE
...
//#endif
			cudaCheckError()
				A4_magnitude_kernel << < gridSize, blockSize, 0, stream[1] >> >(d_dst_c, d_dst_c_mag);
#ifdef DEBUG_MODE
			...
			cudaCheckError()
				...
				//write_to_file_DEBUG(h_ref_c_mag, N);
				A5_fast_lo_stats_kernel << < gridSize, blockSize, 0, stream[1] >> >(d_ref_c_mag, d_ref_Std, d_ref_Skw, d_ref_Krt);
//#ifdef DEBUG_MODE
			...
			write_to_file_DEBUG(h_ref_Std, N);
			cudaMemcpy(h_ref_Skw, d_ref_Skw, REAL_SIZE, cudaMemcpyDeviceToHost);
			write_to_file_DEBUG(h_ref_Skw, N);
			cudaMemcpy(h_ref_Krt, d_ref_Krt, REAL_SIZE, cudaMemcpyDeviceToHost);
			//write_to_file_DEBUG(h_ref_Krt, N);
//#endif
			cudaCheckError()
				A5_fast_lo_stats_kernel << < gridSize, blockSize, 0, stream[1] >> >(d_dst_c_mag, d_dst_Std, d_dst_Skw, d_dst_Krt);
#ifdef DEBUG_MODE
...
#endif
			cudaCheckError()
				A6_delta_stats_kernel << < gridSize, blockSize, 0, stream[1] >> >(d_ref_Std, d_ref_Skw, d_ref_Krt, d_dst_Std, d_dst_Skw, d_dst_Krt, scale[s] / 13.25f, d_eta);
